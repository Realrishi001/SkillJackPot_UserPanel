...
export default function Page() {
  ...
  async function fetchTickets() {
    try {
      setLoading(true);
      const loginId = "4"; // Replace with the dynamic loginId
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left">
              ...
              <tbody className="divide-y divide-slate-600/20">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-16 text-center text-slate-400">Loading...</td>
                  </tr>
                ) : showData.length === 0 ? (
                  <tr>
                  ...
                  showData.map((row) => (
                    <tr key={row.sr} className="hover:bg-slate-700/30 transition-all duration-200 group">
